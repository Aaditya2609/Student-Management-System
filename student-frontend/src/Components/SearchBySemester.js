...
import "../styles/SearchBySemester.css"
import StudentService from "../Services/StudentService";
import Loader from "./Loader";
import ReactPaginate from 'react-paginate';
export default class SearchBySemester extends Component {
    ...
    handleChange = async (e) => {
        await this.setState({ studentSemester: e.target.value });
    }

    handleSubmit = (e) => {
        e.preventDefault();
        if (this.state.studentSemester) {
            ...
            StudentService.getBySemester(this.state.studentSemester).then((response) => {
                ...
                if ((response.data.response) == null) {
                    ...
                    this.setState({ showForm: true, foundStudents: response.data.response, notFound: false, found: true, showLoader: false, showFullDetails: false, deleted: false });
                    this.receivedData();
                }
            });
        }
    ...
    receivedData = () => {
        ...
        const postData = slice.map(value => <React.Fragment>
            <tr className="Searchrow">
                ...
                <td className="SearchData BranchData">{value.semester}</td>
    ...
    goBack = () => {
        this.setState({ deletePrompt: false, showForm: true, deleted: false, found: false, notFound: false, showLoader: false, showFullDetails: false, studentSemester: "" });
    }
    showDeletePrompt = () => {
        ...
        return (
            <>
                {this.state.showForm && (
                    <div id="searchContainer" className="searchContainer">
                        <form className="searchForm">
                            <h1 className="heading">Enter Student Semester</h1>

                            <input type="number" required name="studentSemester" id="studentId" value={this.state.studentName} onChange={this.handleChange} placeholder=" Student Semester : " />
                            <div className="error" style={{ color: "red" }}>{this.state.semesterError}</div>
                ...
                {(this.state.notFound && !this.state.found) && (
                    <div id="notFound">
                        <p className='notFoundText'>No Student/s in this Semester</p>
                    </div>
                )}
                    ...
                    <div className="foundContainer" id="foundContainer" style={{ flexDirection: "column" }}>
                        <div className="result">
                            <tr className="Searchrow">
                                ...
                                <th className="SearchHead Branch">Semester</th>
                            ...
                            <div className="totalCount"><i class="fa fa-archive" style={{ fontSize: "27px", marginRight: "10px" }} />Total Students enrolled in this Semester: {this.state.foundStudents.length}</div>
                        </div>
                    </div>
                )}
            </>
